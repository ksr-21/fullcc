...
import { getCourses, createCourse, updateCourse, addCourseNote, takeAttendance } from '../controllers/academicController.js';
const router = express.Router();
router.route('/courses').get(protect, getCourses).post(protect, authorize('Teacher', 'HOD/Dean', 'Director'), createCourse);
router.route('/courses/:id').put(protect, authorize('Teacher', 'HOD/Dean', 'Director'), updateCourse);
