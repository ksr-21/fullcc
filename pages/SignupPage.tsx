    ...
    const handleSignup = async (e: React.FormEvent) => {
        ...
        try {
            ...
            try {
                ...
                if (authErr.message?.includes('No invitation found for this email')) {
                    const seeded = await auth.seedSuperAdmin(name.trim() || cleanEmail.split('@')[0], cleanEmail, password);
                    if (seeded?.user?.tag === 'Super Admin') {
