...
const getCellSubjectIds = (cell: any) => {
    if (!cell) return [] as string[];
    if (cell.subjectIds && cell.subjectIds.length > 0) return cell.subjectIds;
    if (cell.subjectId) return [cell.subjectId];
    return [];
};
        ...
        for (const [classId, classSchedule] of Object.entries(college.timetable)) {
            ...
            if (cell && cell.type !== 'practical' && subjectIds.length > 0) {
                ...
                if (effectiveFacultyIds.includes(currentUser.id)) {
                    ...
                    const subjectLabel = subjectIds.map(id => courses.find(c => c.id === id)?.subject).filter(Boolean).join(' / ');
                    mySessions.push({
                        classId,
        ...
        const rows = slots.map(slot => {
            ...
            days.forEach(day => {
                ...
                if (!session) {
                    ...
                    const subjects = subjectIds.map(id => courses.find(c => c.id === id)?.subject).filter(Boolean).join(' / ');
                    row.push(`"${subjects}"`);
                }
            });
            return row.join(',');
                            ...
                            <tr key={slot.id} className="group">
                                ...
                                {days.map((day) => {
                                    ...
                                    const subjectLabel = subjectIds.length > 0 
                                        ? subjectIds.map(id => courses.find(c => c.id === id)?.subject).filter(Boolean).join(' / ') 
                ...
                if (cell) {
                    ...
                    if (cell.type !== 'practical') {
                        ...
                        if (fIds.includes(currentUser.id)) {
                            ...
                            subjectLabel = cellSubjectIds.map(id => courses.find(c => c.id === id)?.subject).filter(Boolean).join(' / ');
                            room = cell.roomId || '';
                        }
                    } 
