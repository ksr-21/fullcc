...
const SuperAdminPage: React.FC<SuperAdminPageProps> = ({ 
  ...
  const toggleStatus = async (collegeId: string, currentStatus: CollegeStatus) => {
      const newStatus = currentStatus === 'suspended' ? 'active' : currentStatus === 'pending' ? 'active' : 'suspended';
      if (window.confirm(`Are you sure you want to ${newStatus === 'suspended' ? 'suspend' : 'activate'} this college?`)) {
