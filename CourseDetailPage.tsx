--- START OF FILE Paste January 25, 2026 - 4:41PM ---
...
const CourseDetailPage: React.FC<CourseDetailPageProps> = ({
    ...
    useEffect(() => {
        ...
        const record = course.attendanceRecords?.find(r => new Date(r.date).toDateString() === selectedDateStr);
    ...
    const handleTakeAttendance = () => {
        ...
        const record: AttendanceRecord = { date: timestamp, records: {} };
    ...
    const downloadAttendanceRegister = () => {
        ...
        // Use UTC iteration for robustness since input[type="date"] gives YYYY-MM-DD
        const datesInRange: { iso: string, display: string }[] = [];
        let tempDate = new Date(exportFromDate);
        const endDate = new Date(exportToDate);
        
        // Guard against invalid ranges
        if (isNaN(tempDate.getTime()) || isNaN(endDate.getTime()) || tempDate > endDate) {
            alert("Invalid date range selected.");
            return;
        }

        // Loop through the selected range to create columns
        while (tempDate <= endDate) {
            const iso = tempDate.toISOString().split('T')[0];
            const d = tempDate.getUTCDate();
            const m = tempDate.getUTCMonth() + 1;
            datesInRange.push({
                iso: iso,
                display: `${d}/${m}`
            });
        ...
        const csvHeaders = ['roll no.', 'name', ...datesInRange.map(d => d.display), 'percentage'];
        ...
        sortedStudents.forEach(s => {
            ...
            datesInRange.forEach(dateObj => {
                // Find record where date matches (comparing ISO strings)
                const record = course.attendanceRecords!.find(r => 
                    new Date(r.date).toISOString().split('T')[0] === dateObj.iso
                );
                
                if (record && record.records[s.id]) {
                    ...
                    row.push(''); // No record for this date
                }
            });
            ...
            <main className={`flex-1 ${!isEmbedded ? 'container mx-auto px-4 py-8 pb-24 lg:pb-8' : ''}`}>
                <div className="max-w-6xl mx-auto">
                    ...
                    <div className="bg-gradient-to-br from-indigo-700 via-indigo-900 to-slate-900 rounded-[2.5rem] p-8 md:p-12 text-white shadow-2xl relative overflow-hidden mb-8 border border-white/5">
                        ...
                        <div className="relative z-10 flex flex-col md:flex-row justify-between gap-8 items-start md:items-center">
                            <div className="flex-1 min-w-0">
                                <button onClick={() => onNavigate('#/academics')} className="flex items-center gap-2 text-xs font-black text-indigo-300 uppercase tracking-widest mb-6 hover:text-white transition-colors group">
                                    <ArrowLeftIcon className="w-4 h-4 transition-transform group-hover:-translate-x-1"/> Back to Hub
                                </button>
                    ...
                    <div className="min-h-[400px]">
                                        ...
                                        <div className="bg-card p-6 rounded-[2rem] border border-border shadow-sm">
                                            ...
                                            <div className="space-y-4">
                                                <div className="p-4 bg-muted/50 rounded-2xl border border-border">
                                                    ...
                                                    <input type="date" value={attendanceDate} onChange={e => setAttendanceDate(e.target.value)} className="w-full bg-transparent font-black text-foreground focus:outline-none mt-1" />
                                                </div>
                        ...
                        {activeTab === 'people' && (
                             <div className="animate-fade-in grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                                {students.map(s => (
                                    <div key={s.id} onClick={() => onNavigate(`#/profile/${s.id}`)} className="bg-card p-4 rounded-3xl border border-border/60 hover:shadow-md transition-all flex flex-col items-center text-center group cursor-pointer">
                                        <Avatar src={s.avatarUrl} name={s.name} size="xl" className="w-20 h-20 mb-3 group-hover:scale-110 transition-transform shadow-md" />
                                        <h5 className="font-black text-foreground truncate w-full">{s.name}</h5>
