        ...
        const rows = DAYS.map(day => {
            ...
            effectiveSlots.forEach(slot => {
                ...
                if (!cell) {
                    ...
                    const subjects = subjectIds.map(id => relevantCourses.find(c => c.id === id)?.subject).filter(Boolean).join(';');
                    row.push(subjects);
                }
            });
            return row.join(',');
                                                            ...
                                                            <div className="mt-2 flex flex-wrap gap-1">
                                                                {getCellSubjectIds(cell).length > 0 && (
                                                                    <div className="text-[9px] font-black text-indigo-600 uppercase tracking-widest bg-indigo-500/10 px-2.5 py-1 rounded-full border border-indigo-500/20 shadow-sm">
                                                                        {getCellSubjectIds(cell).map(id => relevantCourses.find(c => c.id === id)?.subject).filter(Boolean).join(' / ')}
                                                                    </div>
                                                                )}
                                                            </div>
                                                        </div>
                                    ...
                                    <tr key={slot.id}>
                                        ...
                                        {DAYS.map(day => {
                                            ...
                                            const subject = subjectIds.length > 0
                                                ? subjectIds.map(id => relevantCourses.find(c => c.id === id)?.subject).filter(Boolean).join(' / ')
                                                : null;
