...
const ChatPanel: React.FC<ChatPanelProps> = ({ conversation, currentUser, users, onSendMessage, onDeleteMessagesForEveryone, onDeleteMessagesForSelf, onClose, onNavigate }) => {
  ...
  const directOtherUser = otherParticipantId ? users[otherParticipantId] : null;
  const chatName = isGroupChat ? conversation.name : directOtherUser?.name;
  ...
  if (!isGroupChat && !directOtherUser) {
    ...
    <div className="flex flex-col h-full bg-muted/30">
        ...
        <div className="p-3 border-b border-border flex items-center justify-between bg-background/80 backdrop-blur-md sticky top-0 z-10">
            <div className="flex items-center space-x-3 flex-1 overflow-hidden">
                ...
                {isGroupChat ? (
                    ...
                    <div className="cursor-pointer" onClick={() => onNavigate(`#/profile/${directOtherUser!.id}`)}>
                        <Avatar src={directOtherUser!.avatarUrl} name={directOtherUser!.name} size="md" />
                    </div>
                )}
                <div className={!isGroupChat && directOtherUser ? "cursor-pointer flex-1 overflow-hidden" : "flex-1 overflow-hidden"} onClick={!isGroupChat && directOtherUser ? () => onNavigate(`#/profile/${directOtherUser.id}`) : undefined}>
                    ...
                    {isGroupChat ? (
                        ...
                        <p className="text-xs text-muted-foreground truncate">{directOtherUser!.department}</p>
                    )}
                </div>
            </div>
        </div>
      ...
      <div ref={messagesContainerRef} className="flex-1 overflow-y-auto no-scrollbar p-4 space-y-2 chat-background-panel">
        {visibleMessages.map((msg, index) => {
          ...
          const mediaUrl = msg.mediaUrl || msg.image;
