...
type ParsedTeacher = { name: string; email: string; designation?: string; qualification?: string; error?: string; };
...
const AddTeachersCsvModal: React.FC<AddTeachersCsvModalProps> = ({ isOpen, onClose, department, onCreateUsersBatch, existingEmails = [] }) => {
    ...
    const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = (e) => {
            ...
            const designationIndex = header.indexOf('designation');
            ...
            const data = lines.map((line): ParsedTeacher => {
                ...
                const designation = designationIndex !== -1 ? values[designationIndex]?.trim() : '';
                const qualification = qualificationIndex !== -1 ? values[qualificationIndex]?.trim() : '';
                let error;
                ...
                return { name, email, designation, qualification, error };
            });
    ...
    const CsvTemplateLink = () => {
        const csvContent = "name,email,designation,qualification\nJane Doe,jane.doe@example.com,Professor,PhD\nJohn Smith,john.smith@example.com,Lecturer,M.Tech";
                ...
                return (
                    <div className="text-center">
                        ...
                        <div className="mt-4 text-sm text-text-muted">
                            <p>File must have 'name', 'email' columns. 'designation' and 'qualification' are optional.</p>
                            <CsvTemplateLink />
                        </div>
                    </div>
                );
                            ...
                            <table className="w-full text-sm">
                                <thead className="sticky top-0 bg-slate-100 dark:bg-slate-700">
                                    <tr>
                                        ...
                                        <th className="p-2 text-left font-semibold">Designation</th>
                                ...
                                <tbody>
                                    {parsedData.map((row, index) => (
                                        <tr key={index} className="border-t border-border">
                                            ...
                                            <td className="p-2 text-xs">{row.designation || '-'}</td>
                                            <td className="p-2">
                                                {row.error ? 
